...
function renderSectionGrid(){
 ...
 grid.innerHTML=visible.length?visible.map(s=>{
  ...
  const a=aggregate([s]),fill=voteHeat(a);
  const rgb=[1,3,5].map(i=>parseInt(fill.slice(i,i+2),16));const ink=rgb[0]*.299+rgb[1]*.587+rgb[2]*.114>155?'#332c30':'#fff';
  return `${group}<button class="section-tile${state.section===s.id?' selected':''}" style="--tile-color:${fill};--tile-ink:${ink}" data-section-id="${esc(s.id)}" aria-haspopup="dialog" aria-label="Zona ${esc(s.zone)}, seção ${esc(s.section)}, ${esc(neighborhoodName(s))}. Ver resultado"><span>Z ${esc(String(s.zone).padStart(2,'0'))}</span><strong>${esc(s.section.padStart(4,'0'))}</strong></button>`;
