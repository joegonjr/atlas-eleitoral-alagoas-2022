...
(async()=>{const browser=await chromium.launch({headless:true,channel:'msedge'});try{
 ...
 await page.locator('#neighborhood').fill('Farol');await page.locator('#neighborhood').press('Enter');await page.locator('#map-in').click();assert(await page.evaluate(()=>echarts.getInstanceByDom(document.querySelector('#map-chart')).getOption().geo[0].zoom>1));
 await page.locator('#reset').click();await page.locator('#city').fill('Água Branca');await page.locator('#city').press('Enter');assert((await page.locator('.map-tools').innerText()).includes('indisponível'));assert(await page.locator('#neighborhood').isEnabled());
